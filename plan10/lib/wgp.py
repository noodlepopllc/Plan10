...
f''' <Picture 1> is the first frame of [Shot 1] static. The first frame of the video must match <Picture 1> exactly, including identical pose, head angle, hand position, body orientation, facial expression, and clothing folds, with zero deviation. {cam_desc} Camera focuses on <Subject 1> as they speak with initial framing, keeping them clearly in frame. The character faces the camera and speaks, with precise lip movements, jaw adjustments, and subtle facial micro-expressions perfectly synchronized to the cadence and dialogue of <Audio 1>.  \n'''
f''' After speaking, <Subject 1> {prompt} {"They continue to move naturally for the remainder of the video transitioning into <Picture 2> is the last frame of [Shot 1] static. After transitioning fully into <Picture 2>, the subject holds completely still with no additional motion for the remainder of the clip." if end_image else ''} static \n overall_soundscape: {audio_desc} ''') 
