    ...
    try:
        ...
        if vram_limit < 32:
            ...
            pipe = MiniMaxH3Pipeline.from_pretrained(
                ...
                device="cuda",
                model_configs=[
                    ModelConfig(model_id="MiniMax/MiniMax-H3", origin_file_pattern="Ref2VA/text_encoder/model*.safetensors", quantize=QuantizeConfig(method="torchao_nvfp4_w4a4"), **vram_config),
                    ModelConfig(model_id="Comfy-Org/MiniMax-H3", origin_file_pattern="diffusion_models/minimax_h3_ref2va_pruned_bf16.safetensors",  quantize=QuantizeConfig(method="torchao_nvfp4_w4a4"), **vram_config),
                    ModelConfig(model_id="MiniMax/MiniMax-H3", origin_file_pattern="Ref2VA/video_vae/source/model.safetensors",  quantize=QuantizeConfig(method="torchao_nvfp4_w4a4"), **vram_config),
                    ModelConfig(model_id="MiniMax/MiniMax-H3", origin_file_pattern="Ref2VA/audio_vae/model.safetensors",  quantize=QuantizeConfig(method="torchao_nvfp4_w4a4"), **vram_config),
                ],
                processor_config=ModelConfig(model_id="MiniMaxAI/MiniMax-H3", origin_file_pattern="Ref2VA/processor/"),
                vram_limit=vram_limit,
            )
